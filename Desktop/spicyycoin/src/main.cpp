...
#if defined(NDEBUG)
# error "Spicyycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x12a14e942128808ca9a41fcfffcb61668f61c72a7e8f920248ed2b059f45e64e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Spicyycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Spicyycoin Signed Message:\n";
...
{
    // Spicyycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Spicyycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Spicyycoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Spicyycoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Spicyycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Spicyycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1b98204aa84f513c13ec5632597b81837a1fc6072ac50e2f85cfed51b74027fa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Twitch streamer SpicyyDoggo released the best cryptocurrency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0490cb496fb0a811d2ff895d73a6729f162d8e90421e144c55ea7491a1ad4a26b5fcb6dac323bacf3df9d52c7e251dbc5c401ada9f480998a8b7d747d48ebe1af0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517840916;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086043079;

        if (fTestNet)
        {
            block.nTime    = 1517840896;
            block.nNonce   = 385795388;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x28e4d7603e80642ebc350b7d6f6d2f6cf88de860483312ff6b43cd63e7a24924"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc7, 0xbc, 0xd9 }; // Spicyycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpicyycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpicyycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpicyycoinMiner(CWallet *pwallet)
{
    printf("SpicyycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spicyycoin-miner");
    ...
    try { loop {
        ...
        printf("Running SpicyycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpicyycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpicyycoinMiner, pwallet));
}
